    ...
    def _fallback_parse(self, text: str) -> Optional[Dict]:
        ...
        def grab_duration(keywords: list) -> str:
            for kw in keywords:
                ...
                pattern_mins_plural = rf"(?i){kw_esc}[\s]*[:=-][\s]*(\d+\s*mins?)"
                match = re.search(pattern_mins_plural, text)
                if match:
                    return match.group(1).strip()

                # Handle "39 min" format (minutes with spaces, singular)
                pattern_min_singular = rf"(?i){kw_esc}[\s]*[:=-][\s]*(\d+\s*min)"
                match = re.search(pattern_min_singular, text)
                if match:
                    return match.group(1).strip()
